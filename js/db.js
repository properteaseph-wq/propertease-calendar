...
function oldMonthKey(yyyymm) {
  return `${NS_OLD}.month.${yyyymm}`;
}

function oldLastMonthKey() {
  return `${NS_OLD}.lastMonth`;
}
...
/**
 * One-time migration:
 * - pulls months stored under old localStorage keys
 * - writes them into IndexedDB
 * - marks migrated flag in meta store
 */
async function migrateLocalStorageToIDB(db) {
  ...
  // Import all found months
  for (const item of monthsToImport) {
    await put(db, STORE_MONTHS, item);
  }

  // Import lastMonth
  const last = localStorage.getItem(oldLastMonthKey());
  if (last) {
